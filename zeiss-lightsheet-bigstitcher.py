...
from loci.formats import ImageReader, MetadataTools, TileStitcher
...
def check_fusion_settings(czi_path):
    ...
    # h5_filesize = os.path.getsize(fused_xml_path + ".h5")
    ...
    sufficient_ram = ram_requirement < free_memory / 1.5
    ...
    if not sufficient_ram:
        try:
            ...
            if yn.yesPressed():
                fuse_tiff = False
            elif yn.cancelPressed():
                sys.exit("Cancel button pressed")
            else:
                do_fusion = False
        except Exception:
...
def convert_to_imaris(input_path, output_path):
    """Convert a file to the Imaris file formats

    Will also try to parse multiple channels and timepoints

    Parameters
    ----------
    input_path : str
        Path to the first file to be converted
    output_path : str
        Path to save the newly created IMS
    """
    ...
    subprocess.call(command, shell=True, cwd=locate_latest_imaris())
...
input_path = input_path.getPath()
first_czi = input_path
...
fused_xml_path = temp + "/" + project_filename_short
fused_xml_path = first_czi.replace(".czi", "_fused.xml")
fused_h5_path = first_czi.replace(".czi", "_fused.h5")
...
# print("time to get calibration [HH:MM:SS.ss] " + str(get_cal_time))
...
dataset_def_time = time.time()
print(
    "time to define dataset [HH:MM:SS.ss] " + elapsed_time_since(execution_start_time)
)
...
if not convert_to_ims:
    shutil.copy2(project_path, fused_xml_path)
...
resave_time = time.time()
print(
    "time to resave dataset to h5/xml [HH:MM:SS.ss] "
    + elapsed_time_since(dataset_def_time)
)
...
IJ.run(
    ...
    + "how_to_treat_illuminations=compare "
...
print("time to register tiles [HH:MM:SS.ss] " + elapsed_time_since(resave_time))
...
if fuse:
    ...
    print("time to fuse dataset [HH:MM:SS.ss] " + elapsed_time_since(registration_time))
...
if fuse and convert_to_ims and imaris_path:
    if fuse_tiff:
        ...
        file_to_convert_to_ims = fused_xml_path
    # imaris_voxelsize = "%s-%s-%s" % (first_czi_calibration[0], first_czi_calibration[1], first_czi_calibration[2])

    convert_to_imaris(file_to_convert_to_ims, first_czi.replace(".czi", ".ims"))

    convert_to_ims_time = time.time()
    print(
        "time to convert dataset to ims [HH:MM:SS.ss] "
        + elapsed_time_since(fusion_time)
    )
